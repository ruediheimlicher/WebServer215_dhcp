...
void client_http_post(char *urlbuf, char *hoststr, char *additionalheaderline,char *postval,void (*callback)(uint8_t,uint16_t))
{
	client_urlbuf=urlbuf;
	client_hoststr=hoststr;
	...
	if ( buf[TCP_DST_PORT_H_P]==TCPCLIENT_SRC_PORT_H){
		...
		if ((buf[TCP_FLAGS_P] & TCP_FLAGS_SYN_V) && (buf[TCP_FLAGS_P] &TCP_FLAGS_ACK_V)){
			...
			if (browsertype==0)
			{
				...
				len=fill_tcp_data_p(buf,len,client_urlbuf); //			/blatt/cgi-bin/home.pl?pw=
				
				len=fill_tcp_data(buf,len,client_urlbuf_var); //		ideur00
				len=fill_tcp_data_p(buf,len,PSTR(" HTTP/1.0\r\nHost: "));
				len=fill_tcp_data_p(buf,len,client_hoststr);	//		www.schuleduernten.ch
				//lcd_puts(buf);
				//lcd_delay_ms(200);
